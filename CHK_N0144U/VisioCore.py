...
class CenterByTemplate:
    def __init__(self):
        self.sizes = [n for n in range(200, 240, 5)]
...
class Evaluate:
    def __init__(self):
        ...
        self.fontScale = 0.5
        self.lineType = 1
        self.scores_pos = {"vnitrni": (35, 60), "hranice": (35, 30), "type": (35, 90)}

        cv2.putText(self.blank, "hranice:", (1, 15), self.font, self.fontScale, self.blue, self.lineType)
        cv2.putText(self.blank, "vnitrni:", (1, 45), self.font, self.fontScale, self.blue, self.lineType)

    def compute(self, in_circle, dscore):
        dscore = int(dscore / 1020 * 100)
        self.eval_res = cp(self.blank)
        OK = in_circle > dscore

        if OK:
            cv2.putText(self.eval_res, str(in_circle), self.scores_pos["vnitrni"], self.font, self.fontScale, self.green, self.lineType)
            cv2.putText(self.eval_res, str(dscore), self.scores_pos["hranice"], self.font, self.fontScale, self.green, self.lineType)
            cv2.putText(self.eval_res, "N0144U", self.scores_pos["type"], self.font, self.fontScale, self.green, self.lineType)
        else:
            cv2.putText(self.eval_res, str(in_circle), self.scores_pos["vnitrni"], self.font, self.fontScale, self.red, self.lineType)
            cv2.putText(self.eval_res, str(dscore), self.scores_pos["hranice"], self.font, self.fontScale, self.red, self.lineType)
            if in_circle > 0:
                cv2.putText(self.eval_res, "CHK", self.scores_pos["type"], self.font, self.fontScale, self.red, self.lineType)
