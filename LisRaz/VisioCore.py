...
class Evaluate:
    ...
    def compute(self, is_circle):
        self.eval_res = cp(self.blank)
        if is_circle:
            cv2.putText(self.eval_res, "RAZOVANA", (5, 60), self.font, self.fontScale, self.green, self.lineType)
        else:
            cv2.putText(self.eval_res, "LISOVANA", (5, 60), self.font, self.fontScale, self.red, self.lineType)
...
class FindCircle:
    def __init__(self):
        self.circled = np.zeros((100, 100), np.uint8)

    def compute(self, img, min_rad, max_rad, min_dist):
        ...
        if circles is not None:
            circle_cent = int(circles[0][0][0]), int(circles[0][0][1])
            circle_rad = int(circles[0][0][2])
            img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
            cv2.circle(img, center=circle_cent, radius=circle_rad, color=(0, 255, 0), thickness=3)
            is_circle = True
